...
public class ResortController {
	...
	/*
	 * RedirectAttributes
	 * -> Model 데이터는 redirect 전송 시 사라짐.
	 * Redirect가 발생하면 새로운 요청이 생성되고, 이 새로운 요청에서는 이전 요청에서의 모델 데이터가
	 * 포함되지 않음. 이를 보완하기 위해서는 Flash Attribute를 사용하는데, Flash Attribute는
	 * redirect 후에도 데이터를 유지할 수 있음. 
	 * 대표적인 예로 RedirectAttributes 사용 
	 */
	@PostMapping("/d_02_1")
