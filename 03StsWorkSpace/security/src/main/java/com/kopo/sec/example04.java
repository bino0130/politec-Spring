...
public class example04 {
	...
	/*
	 * <form-login> 태그
	 * : 인증되지 않은 사용자가 특정 경로에 접근할 때 인증이 필요하면 로그인 페이지를 출력
	 * 
	 ...
	 * authentication-failure-url = 로그인 실패 시 이동할 경로
	 * always-user-default-target = boolean 형으로 true로 설정하면 설정된 
	 * 								경로(default-target-url)로 시작
	 * 
	 * logout 태그
	 * delete-cookies = 로그아웃에 성공할 때 삭제할 쿠키 이름
	 * invalidate-session = 로그아웃할 때 세션 제거 여부 설정, default값은 true(제거한다는 뜻) 
	 * logout-success-url = 로그아웃하고 이동할 페이지, default값은 /login?logout
